...
def validate_problem_payload(input_data):
    """
    Validates and structures the problem payload correctly before sending to ServiceNow.
    """
    if isinstance(input_data, str):
        try:
            ...
            logging.error(f"🚨 JSON decoding error: {e} | Received input: {input_data}")
            return False, "Invalid JSON format received."
    else:
        parsed_data = input_data

    short_desc = parsed_data.get("short_description", "").strip()
    
    # Ensure short_description is provided
    ...
    problem_payload = {
        ...
        "description": parsed_data.get("description", "No additional details provided."),
        "category": parsed_data.get("category", "Network"),
        "subcategory": parsed_data.get("subcategory", "Connectivity"),
        "impact": parsed_data.get("impact", "2"),
        "urgency": parsed_data.get("urgency", "2"),
        "priority": parsed_data.get("priority", "4"),
        "problem_state": "101"
    }

    return True, problem_payload
